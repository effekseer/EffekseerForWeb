...
| Purpose / 目的 | WebGL | WebGPU |
| --- | --- | --- |
| Runtime / runtime | `backend: "webgl"` with `effekseer-webgl.js` / `.wasm` | `backend: "webgpu"` with `effekseer-webgpu.js` / `.wasm` and `GPUDevice` |
| Context target / 描画先 | `graphicsContext` from `canvas.getContext("webgl2")`, `webgl`, or `OffscreenCanvas` | `canvasContext` from `canvas.getContext("webgpu")` or `OffscreenCanvas`, plus `device` |
| Normal draw loop / 通常描画 | `context.update(1); context.draw();` | `context.update(1); context.draw();` |
| App-owned render pass / アプリ側 render pass | Not provided | `context.drawToRenderPass(renderPassEncoder, options)` |
...
For WebGL, a WebGL context created from `OffscreenCanvas` can be passed through the same `graphicsContext` option. In a Worker, provide the native module factory through `moduleFactory` instead of using `scriptPath`, which depends on document script loading.
...
WebGPU でも `OffscreenCanvas` から `webgpu` context を作成し、`canvasContext` として渡せます。通常の `draw()` はその `GPUCanvasContext` の current texture へ描画します。Worker 内では WebGL と同様に `moduleFactory` を使って native module factory を渡してください。

For WebGPU, you can also create a `webgpu` context from `OffscreenCanvas` and pass it as `canvasContext`. The normal `draw()` path renders to that `GPUCanvasContext` current texture. In a Worker, use `moduleFactory` to provide the native module factory, as with WebGL.
